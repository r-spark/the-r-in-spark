...
If you ar enew to Kafka, we don't recommend you try to run the code from this section; but if you are really motivated to follow along , first you will need to install Kafka as explainede in the Appendix or deploy Kafka in your cluster.

Using Kafka also requires the Kafka package wheh connecting to Spark, make sure is specified in your connection `config`:
...
However, notice that you need to properly configure the `options` list; `kafka.bootstrap.server` expects a list of Kafka hosts, `topic` and `subscribe` define which topic should be used when writing or readring from Kafka respeectevely.
...
stream_read_kafka(sc, options = read_options) %>%
  ...
  mutate(value = paste0(value, "=", n)) %>%       # kafka expecet a value field
...
server <- function(input, output, session) {
  iris <- reactiveSpark(stream_read_csv(sc, "shiny-stream"))
...
Then we closed this chapter with "a cherry on top of the cake" by presenting how to use Spark streams in Shiny, which to our surprise, was quiote easy to accomplish since a stream can be transformed into a reactive, which is the lingua franca in the world of reactivity.

It is time now to move to our very last and quite short chapter, Contributing; where we will try to recruit you -- brainwash you -- into using your newly acquired knowledge for the bennefit of Spark and R community at large.
