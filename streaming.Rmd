...
Each sub-section will cover an option provided by `sparklyr` to perform trasnformations on a stream. 
...
The `reactiveSpark()`, mentioned in the Shiny Reactive section ealier in this chapter,  provides a mechanism to process the transformations on a stream. It lets us circumvent the need for writing an output.  Also, because it does not depend on the stream writing functions, it is possible to to use watermark groups. 
...
In a more complex implemenation, the source would more likely be a Kafka stream.  The next section will cover how to integrate Kafka, Spark Stream and `sparklyr`.
