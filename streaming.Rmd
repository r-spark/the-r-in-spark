...
Figure 2.6 is a diagram of how this example will work. The **Producer** will stream random, single letters of the alphabet into Kafka. The topic will be called "letters". A **Stream Processor** will be built in Spark by having it read the "letters" topic, and then produce the count of how many letter A's are in the batch.  The results are written to another topic, called "a-only".  The same Spark connection will be used to setup a **Consumer** that reads the 'a-only' topic.
...
[Producer | Streams 'letters'] -> [Kafka | Stores 'letters' & 'a-only']
[Kafka] <-> [Stream Processor 
(Spark)| Subscribes to 'letters'
Writes results to 'a-only']
[Kafka] -> [Consumer
(Spark) | Subscribes to 'a-only']
...
```
```{r echo=FALSE, out.width='100%', out.height='220pt', fig.cap='Stream Processor (Spark)', fig.align = 'center', eval = TRUE}
render_nomnoml("
#direction: right
#padding: 10
#fontSize: 14
...
['letters' topic |
[timestamp 1 | V A O E Q W H K T R A ]
[timestamp 2 | P T G H T X F A A X A D B Y ]
] -> [Stream Processor | Spark Streaming]
[Stream Processor] -> ['a-only' topic | 
[timestamp 1 | 1 ] 
[timestamp 2 | 3]
]
...
1. The local Kafka cluster is served on port 9092, by default.  In order to keep the read and write calls a little cleaner, a couple of variables will contain the Kafka options.  Notice that the read option has `subscribe`, while the write option contains `topic`.  
    ...
    write_options <- list(kafka.bootstrap.servers = hosts, topic = "a-only")
    ```
...
1. This step takes the data from the "letters" topic groups them by the timestamp, and summarises the number of A's found.  It is important to note that the results are writen to a character variable called 'value'.  Kafka expect at least one variable with this name, value.  It also considers its contents to be character, so it is better to perform any type coercion before writing back to Kafka.
    ```{r}
    step_2 <- step_1 %>%
      mutate(value = as.character(value), key = as.character(key)) %>%
      #group_by(timestamp) %>%
      summarise(value = sum(ifelse(value == "A", 0 ,1), na.rm = TRUE)) %>%
      mutate(value = as.character(value)) 
    ```

1. Writing back to Kafka is the last step.  As set in a prior step, the topic that the results will write to is called "a-only" 
    ```{r}
    step_3 <- step_2 %>%
      stream_write_kafka(mode = "complete", options = write_options)
    ```
...
1. For purposes of this exercise, a Consumer application is now started. To do that, the data from "a-only" topic will be sent to a Spark in-memory table.  
    ```{r}
    a_only <- stream_read_kafka(
        sc, 
        options = list(
...
1. Text
    ...
    ##   <list>    <list>    <chr>      <int>  <dbl> <dttm>                      <int>
    ## 1 <lgl [1]> <raw [3]> a-only         0      0 2019-04-26 21:08:49             0
    ## 2 <lgl [1]> <raw [3]> a-only         0      1 2019-04-26 21:08:49             0
    ## 3 <lgl [1]> <raw [3]> a-only         0      2 2019-04-26 21:08:49             0
    ## 4 <lgl [1]> <raw [3]> a-only         0      3 2019-04-26 21:08:49             0
    ## 5 <lgl [1]> <raw [3]> a-only         0      4 2019-04-26 21:08:49             0
    ## 6 <lgl [1]> <raw [4]> a-only         0      5 2019-04-26 21:08:49             0
    ```
    ```{r}
    tbl(sc, "only_a") %>%
      head() %>%
    ...
    ## # Source: spark<?> [?? x 7]
    ##   key       value topic  partition offset timestamp           timestampType
    ##   <list>    <chr> <chr>      <int>  <dbl> <dttm>                      <int>
    ## 1 <lgl [1]> 2.0   a-only         0      0 2019-04-26 21:08:49             0
    ## 2 <lgl [1]> 1.0   a-only         0      1 2019-04-26 21:08:49             0
    ## 3 <lgl [1]> 2.0   a-only         0      2 2019-04-26 21:08:49             0
    ## 4 <lgl [1]> 1.0   a-only         0      3 2019-04-26 21:08:49             0
    ## 5 <lgl [1]> 1.0   a-only         0      4 2019-04-26 21:08:49             0
    ## 6 <lgl [1]> 11.0  a-only         0      5 2019-04-26 21:08:49             0
    ```
