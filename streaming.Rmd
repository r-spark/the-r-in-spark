...
The `reactiveSpark()` provides a mechanism to process the tranformations on a stream. It allows you to circumvent the need for writing an output.  Also, because it does not depend on the stream writing functions, it is possible to to use watermark groups. 
...
1. The *server* function contains a `reactiveSpark()` function.  This function reads the stream, addes the watermark and then performs the aggregation.  The results are then rendered via the *table* output.
    ```{r}
