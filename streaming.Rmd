...
The *infrastructure* of this example as a local, single node Kafka cluster.  The external Producer uses the Kafka CLI to send the stream of letters. The sample code and installation instructions that were used can be found in the Appendix. 
...
The last step starts a Spark job.  The job will remain active until stopped or until Spark disconnects. A this point, there is no visible output. Even if there was an active Producer sending letters over.  
