...
Three modes are available when processing Kafka streams in Spark: _++complete++_, _++update++_, and _++append++_. The `complete` mode provides the totals for every group every time there is a new batch; `update` provides totals for only the groups that have updates in the latest batch; and `append` adds raw records to the target topic. The `append` mode is not meant for aggregates, but works well for passing a filtered subset to the target topic.
