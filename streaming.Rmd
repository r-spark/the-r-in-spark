...
Here are some examples of how `dplyr` transformations will look like when used agaist a streaming source. This section contains a set of quick examples. To reproduce them, you just need to open a local Spark connection and generate some tests.
```{r}
sc <- spark_connect(master = "local")
if(file.exists("source")) unlink("source", TRUE)
...
The `stream_read_csv()` command can be piped into a `dplyr` verb.  In the following case, a `filter()`
```{r}
...
A derived column can be created using `mutate()`
```{r}
stream_read_csv(sc, "source") %>%
  filter(x > 700) %>%
  mutate(y = x / 2) 
...
A basic row count can be also done, in this case using `count()`
```{r}
stream_read_csv(sc, "source") %>%
  filter(x > 700) %>%
  mutate(y = x / 2) %>%
