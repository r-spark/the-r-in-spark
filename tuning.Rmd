...
```{r tuning-overview, echo=FALSE, message=FALSE, fig.align = 'center', out.width='100%', fig.cap='Spark Sorting Overview'}
...
```{r tuning-graph-render, echo=FALSE, fig.align = 'center', fig.cap='Spark Graph Visualization'}
knitr::include_graphics("images/tuning-spark-graph-visualization.png")
```
...
```{r tuning-timeline-shot, echo=FALSE, fig.align='center', out.width='90%', fig.cap='Spark Event Timeline'}
knitr::include_graphics("images/tuning-spark-event-timeline.png")
```
...
```
```{r tuning-submit-run, echo=FALSE, comment=''}
...
```{r tuning-settings-sparklyr, eval=F}
spark_config_settings()
```
```{r tuning-settings-sparklyr-code, echo=F}
...
```{r tuning-partitioning-explicit-billion}
...
```{r tuning-partitioning-explicit-billion-correct}
library(sparklyr)
library(dplyr)
...
```{r tunning-caching-rdd-shot, fig.width = 4, fig.align = 'center', echo=FALSE, fig.cap='Cached RDD in Spark Web Interface.'}
knitr::include_graphics("images/tuning-cache-rdd-web.png")
```
