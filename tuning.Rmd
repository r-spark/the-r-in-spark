...
When tuning a Spark application, the most common resources you would want to request are memory and cores, specifically:
...
When using YARN in Cluster mode, `sparklyr.shell.driver-cores` can be used to configure total cores requested in the driver node, the "Running Spark on YARN" guide provides additional configiuration settings worth familiarizing with.
...
For readability, we've provided the output of this command in table format, replacing the `spark-submit` parameter with the appropriate `spark_config()` setting and removing the parameters that are not aplicable or already presented in this chapter:
...
There is always an implicit partition for each computation that Spark will assign opn it's own. For instance, how your data is distributed in storage will match the default partitions Spark assigns.
