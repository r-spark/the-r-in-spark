...
In the [Modeling](#modeling) chapter, we saw how to build machine learning workflows using high-level functions in sparklyr. In this chapter, we dive into ML Pipelines, which are the engine that powers the features we demonstrated. When you invoke an ML function via the formula interface, for example `ml_logistic_regression(mtcars, am ~ .)`, sparklyr actually constructs a *pipeline* for you under the hood. The Pipelines API is a lower level interface that enables advanced data processing and modeling workflows. In addition, it also facilitates collaboration across data science and engineering teams and the *deployment* of ML models. We wil begin with a few definitions and move on to specific examples of constructing machine learning workflows with pipelines.
