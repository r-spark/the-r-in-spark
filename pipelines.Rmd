...
```{r}
ml_stage(model_2, "logistic_regression")
```
```r
...
It then must be loaded when `spark_connect()` is called; so lets restart your R session, establish a new Spark connection[^4], and load the pipeline model that we previously saved.
...
ml_write_bundle(
  spark_model,
  sample_input = sample_input_tbl, path = "mleap_model.zip")
```

The artifact we just created, `mleap_model.zip`, can now be deployed in any device that runs Java and has the open source MLeap runtime dependencies, without needing Spark or R! To test this model, we can create a new plumber API to expose it. The script `plumber/mleap-plumber.R` is very similar to the previous example:
...
However, before we launch this web API, make sure the runtime dependencies are installed:
...
In this chapter, we discussede Spark Pipelines, which is the engine behind the modeling functions introduced in the previous chapter. We learned how to tidy up our predictive modeling workflows by organizing data processing and modeling algorithms into pipelines. You learned that pipelines also facilitate collaboration among members of a multilingual data science and engineering team by sharing a language agnostic serialization format -- you can export a Spark pipeline from R and let others reload your pipeline into Spark using Python or Scala, which allows them to collaborate without changing their language we choice.

You also learned how to deploy pipelines using `mleap`, a Java runtime which provides another path to productionize Spark models -- you can export a pipeline and integrate it to Java-enabled environments without requiring the targeet environment to support Spark nor R.

You probably noticed that some algorithms, specially the unsupervised learning kind, where slow even for the OKCupid dataset which can be loaded into memory. If we had access to a proper Spark cluster, we could spend more time modeling and less time waiting! Not only that, but we could use cluster resources to run broader hyperparameter-tuning jobs and process larger datasets. To get there, the next chapter will present what exactly a computing cluster is and explain you various options you can consider; like building your own, or using cloud clusters on-demand.
