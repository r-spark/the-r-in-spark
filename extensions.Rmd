...
Using Spark we can create a multi-layer perceptron classifier with `ml_multilayer_perceptron_classifier()` and gradient descent to classify and predict over large datasets. Gradient descent was introduced to layered perceptrons by Geoff Hinton [@extensions-hinton-layered] and like XGBoost, it also makes use of gradient descent. Let's try it out, why we are on it, let's also install `sparktf` which we are also about to introduce.
