...
A perceptron has limited application but layering perceptrons does have non-linear classification capabilities. Using Spark we can create a multi-layer perceptron classifier with `ml_multilayer_perceptron_classifier()` and predict as usual with `ml_predict`. This technique was introduced by Hinton [@extensions-hinton-layered] and like XGBoost, it also makes use of gradient descent.
...
Training large-scale deep learning models is possible in Spark and TensorFlow through frameworks like Horovod. Today, it's possible to use Horovod with Spark from R using the `reticualte` package since Horovod requires Python and Open MPI which goes beyond the scope of this book. Instead, we will introduce an extension in the domain of genomics.
