...
```{r eval=FALSEz}
model <- h2o.glm(x = c("wt", "cyl"), y = "mpg", training_frame = mtcars_h2o, lambda_search = TRUE)
```
```{r extensions-rsparkling, echo=FALSE}
saveRDS(model, "data/09-extensions-h2o-glm.rds")
```

H2O provides additional metrics than Spark MLlib, notice for instance that `Residual Deviance` is provided in the model while this would not be a standard metric when using Spark MLlib:
