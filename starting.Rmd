...
The executors tab provides a view of your cluster resources. For local connections, you will find only one compute instance active; however, in the [tunning](#tunning) you will learn how to request more compute resources. In fact, by default, Spark initializes a local connection using only 500MB of memory, which in practice means that only 384MB are available in Spark for reasons that the tunning chapter will describe and teach how to configure.
...
```
```{r starting-spark-executors-render, eval=TRUE, fig.width=4, fig.align='center', echo=FALSE, fig.cap='Apache Spark Web Interface - Executors Tab.'}
render_image("images/starting-spark-web-executors.png", "Apache Spark Web Interface - Executors Tab.")
```

The last tab to mention is the environment tab which lists all the settings for this Spark application; yet again, the [tunning](#tunning) will introduce them in detail. As you will learn, most settings don't need to be configured explicitly, but in order to properly run at scale, you will have to become familiar with them, eventually.

```{r starting-spark-environment-web-shot, echo=FALSE}
invisible(webshot::webshot(
...
```
```{r starting-spark-environment-render, eval=TRUE, fig.width=4, fig.align='center', echo=FALSE, fig.cap='Apache Spark Web Interface - Environment Tab.'}
...
select(cars, hp, mpg) %>%
  ...
  collect() %>%
  ggplot() +
    geom_point(ggplot2::aes(hp, mpg)) +
    theme_light() +
    ggsave("images/starting-cars-hp-vs-mpg.png", width = 10, height = 6)
```
```{r starting-cars-hp-vs-mpg, eval=TRUE, echo=FALSE, fig.cap='Horse Power vs Miles per Gallon from Motor Trend Car Road Tests.', fig.align = 'center'}
render_image("images/starting-cars-hp-vs-mpg.png", "World’s capacity to store information.")
```

The plot shows that as we increase the horse power in a vehicle, their fuel efficiency measured in miles per gallon gets reduced. While this is insightful, it's hard to predict numerically how increased horse power would affect fuel effiency, modeling can help us overcome this.
...
This model can now be used to predict values that are not in the original datset. For instance, we can add entries for cars with horse power beyond 250 and also visualize the predicted values.
...
model %>%
  ml_predict(copy_to(sc, data.frame(hp = 250 + 10 * 1:10))) %>%
  transmute(hp = hp, mpg = prediction, predicted = "yes") %>%
  full_join(transmute(cars, hp = hp, mpg = mpg, predicted = "no")) %>%
  collect() %>%
  ggplot() +
    geom_point(ggplot2::aes(hp, mpg, color = predicted)) +
    theme_light() +
    scale_color_grey(start = 0.6, end = 0.2) +
    ggsave("images/starting-cars-hp-vs-mpg-prediction.png", width = 10, height = 6)
```
```{r starting-cars-hp-vs-mpg-prediction, eval=TRUE, echo=FALSE, fig.cap='Horse Power vs Miles per Gallon with Linear Predictions.', fig.align = 'center'}
render_image("images/starting-cars-hp-vs-mpg-prediction.png", "World’s capacity to store information.")
```
...
Suppose that we need to randomize our dataset using a well-known technique like R's `jitter` function; in this case, we can apply jitter across the dataset by running custom R code:

```{r}
cars %>% spark_apply(~sapply(.x, jitter))
```
```
# Source: spark<?> [?? x 11]
     mpg   cyl  disp    hp  drat    wt  qsec     vs       am  gear  carb
   <dbl> <dbl> <dbl> <dbl> <dbl> <dbl> <dbl>  <dbl>    <dbl> <dbl> <dbl>
 1  21.0  6.09  160. 110.   3.90  2.62  16.5 -0.138  0.950    3.93 3.92 
 2  21.0  5.76  160. 110.   3.90  2.87  17.0  0.154  1.13     3.91 4.01 
 3  22.8  4.22  108.  93.0  3.85  2.32  18.6  1.00   0.851    3.99 1.03 
 4  21.4  5.70  258. 110.   3.08  3.22  19.4  1.04   0.0536   2.93 1.18 
 5  18.7  8.00  360. 175.   3.15  3.44  17.0  0.182 -0.0256   2.86 2.08 
 6  18.1  5.66  225. 105.   2.76  3.46  20.2  1.02  -0.00437  3.10 0.996
 7  14.3  7.92  360. 245.   3.21  3.57  15.8  0.148  0.166    3.12 4.06 
 8  24.4  4.21  147.  62.1  3.69  3.19  20.0  1.10  -0.192    4.12 2.16 
 9  22.8  3.83  141.  94.8  3.92  3.15  22.9  0.936 -0.0637   3.82 2.14 
10  19.2  6.18  168. 123.   3.92  3.44  18.3  1.04  -0.102    4.00 4.16 
# … with more rows
```

If you are a profficient R users, it can be quite tempting to use `spark_apply()` for everything, but please, don't! `spark_apply()` was designed for advanced use cases where Spark falls short; instead, you will learn how to do proper data analysis and modeling without having to distribute custom R code across your cluster.
