...
**Tip:** When using Windows, we recommend avoiding directories with spaces in their path. If running `getwd()` from R returns a path with spaces, consider switching to a path with no spaces using `setwd("<path>")` or by creating an RStudio project in path with no spaces.
