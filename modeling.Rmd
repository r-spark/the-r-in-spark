...
Beyond linear models, Spark ML supports many of the standard ML algorithms and you can try different algorithms and hyperparameters (values that control the model fitting process) for your problem. You can find a list of supported ML related functions in the [Appendix](#ml-functionlist). The interfaces to access these functionalities are largely identical, so it is easy to experiment with them. For example, to fit a neural network model we can run:
