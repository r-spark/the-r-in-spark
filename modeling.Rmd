...
```{r}
download.file("https://github.com/rudeboybert/JSE_OkCupid/raw/master/profiles.csv.zip", "okcupid.zip")
...
We will be using this dataset in the Pipelines chapter, so lets save it first as Parquet -- an efficient file format ideal for numeric data.
...
We can extract the coefficient estimates into a tidy data frame, which we can then process further, for example, to create a coefficient plot, as shown in Figure \@ref(fig:glr-coefs).
