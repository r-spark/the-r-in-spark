...
We then moved to learn about about unsuperevised learning to process raw text, where you were able to create a topic model that automatically grouped the profiles into six categories. Interestingly, we found out that building the topic model can take a significant amount of time using a single machine.

Perfect timing to start discussing proper Spark clusters, to address the class of problems that could bennefit from additional hardware, like topic modeling. So, where do we get additional machines from? How do we install Spark across them? How do we connect to propere clusters? These are the right questions to keep in mind while we transition to our next chapter, Clusters.
