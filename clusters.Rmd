...
Since `sparklyr` has been deployed in various Cloudera deployments, `sparklyr` got certified with Cloudera[@cloudera-sparklyr-certification], this means that, Cloudera's support is aware of `sparklyr` and can be more effective helping organizations that are using Spark and R, the following table summarizes the versions currently certified with cloudera:
