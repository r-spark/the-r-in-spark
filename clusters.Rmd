...
From within IBM Cloud, open Watson Studio and create a Data Science project, add a Spark cluster under the project settings and launch RStudio from the Launch IDE menu. Please note that the provided version of `sparklyr` was `0.8.4` which is not the latest version released in CRAN.
...
From the Azure portal, the Azure HDInisight service provides support for on-demand Spark clusters. An HDInsight cluster with support for Spark and RStudio can be easily created by selecting the ML Services cluster type; however, the provided version of `sparklyr` and RStudio are not current. Therefore, rather than relying on the automatic configuration, it's strongly encouraged to manually install RStudio and `sparklyr`.
