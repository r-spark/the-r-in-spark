...
Previous chapters used `copy_to()` as a handy helper to copy data from R into Spark; however, `copy_to()` was designed to transfer only small datasets. There are two reasons behind this decision: first, if you are processing large-scale datasets, these datasets are already stored in large-scale storage like HDFS; second, there are already great tools to transfer huge dataset into distributed storage and there are multiple storage solution which complicates thing further. Therefore, transfering large dataset is left to specialized tools like the `hadoop` command line tool.

When using the Hadoop File System as storage in your cluster, you can use `hadoop` to copy files into Spark from the terminal as follows:

```{bash}
hadoop fs -cp large-file.csv /path-in-hdfs/large-file.csv
```

You can then read the file as described in the [File Formats](#data-file-formats) section; for CSV files, you would run:

```{r}
large_file <- spark_read_csv("hdfs:///path-in-hdfs/large-file.csv")
```

More often than not, data will be already stored in the cluster which you can then read as we will describe next.
