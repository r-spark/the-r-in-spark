...
In order to support a broad variety of data source, Spark needs to be able to read and write data in several different **file formats** (CSV, JSON, Parquet, etc), access them while stored in also several **file systems** (HDFS, S3, DBFS, etc) and, potentially, interoperate with other **storage systems** (databases, data warehouses, etc) which this chapter will present; but first, we will start by presenting how to read, write and copy data using Spark.
...
A data lake and Spark usually go hand-in-hand with optional access to storage systems like databases and data warehouses. Presenting all the different storage systems with approrpiate examples would be quite time consuming, so instead we will present some of the commonly used storage sytems.
