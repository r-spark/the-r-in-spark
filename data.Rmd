...
JSON is a file format originally derived from JavaScript which has grown to be language-independent and very popular due to it's flexibility in handling many datasets, include nested datasets.
...
spark_read_source(sc, "data", "data.xml", "xml")
```
```
...
`spark_read_json()` and `spark_write_json()`.
