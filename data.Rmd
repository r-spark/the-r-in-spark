...
In `spark_read_csv()`, `col_spec_1` is passed to the `columns` argument, and `infer_schema` is set to `FALSE`. We will try to match the names and types of the original file.  Also, we will pass the column specification. Doing this will help with performance because Spark will not have to figure out the column types.
...
The following code will crete 10K files adding up to 10GB even if the driver node collecting this dataset had less than 10GB of memory. That said, as explained in the [Analysis](#analysis) chapter, you should avoid collecting large datasets into a single machine since this usually creates a significant performance bottleneck.
