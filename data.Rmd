...
When analyzing data, loading multiple files into a single data object is a common scenario.  In R, we typically use a loop or functional programming to accomplish this.  That is because R has to load each file indidually into the session.
...
The following example shows how to set the field type to something different. However, the new field type needs a compatible type from the original. For example, a `character` field could not be set to `numeric`. If an incompatible type is used, the file read will faill with an error. Additionally, the example also changes the names of the fields.
