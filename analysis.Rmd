...
Visualizing data is a of vital importance to the success of data analysis.  It helps us, as humans, to gain insights by allowing us to derive patterns from data. 
...
In this case, the `mtcars` raw data was **automatically** transformed into three discreete aggregated numbers, then each result was mapped into an `x` and `y` plane, and then the plot was drawn, see *figure 3.8*. As R users, all of the stages of building the plot are conveniently abstracted for us.
...
The key to building plots based on a large data set inside Spark is to separate the data transformation from the mapping and drawing of the plot.   
