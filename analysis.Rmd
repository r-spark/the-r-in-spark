...
Using this conceptual model, lets apply this when using `ggplort2`.
...
db_compute_bins(cars, mpg, binwidth = 3) %>%
  ggplot() + geom_col(aes(mpg, count)) +
  labs(title = "Cars by Miles Per Galon", subtitle = "Histogram over miles per gallon")
```
```{r, eval = FALSE, echo = FALSE}
db_compute_bins(cars, mpg, binwidth = 3) %>%
  ggplot() +
