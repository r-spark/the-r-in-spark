...
Previous chapters focused on introducing Spark with R, they got you up to speed and encouraged you to try basic data analysis workflows. However, they have not properly introduced what such data analysis means, specially while running in Spark. They presented the tools you need throughout this book, to help you spend more time learning and hopefully, less time troublehsooting.

This chapter, Analysis, will introduce tools to perform data analysis in R; which, spoiler alert, are the same tools you will use when using Spark with R! This is not accidental coincidence, but rather, we want data scientist to live in a world where technology is hidden from them, where you can use the R packages you know and love, and where they simply happen to just work across! Now, we are not quite there yet, but we are also not that far. This chapter will present the obvious tools to do data analysis in R that also apply to Spark: `dplyr`, `ggplot2`, formulas, `rmarkdown` and so on.

The next chapter, Modeling, will focus on creating statistical models to predict, estimate and describe datasets; but first, lets get out data analysis going!
...
Commonly, in R all of the steps are performed in local memory.  But that approach has to change when the analysis adds the use of Spark.
...
```{r analysis-approach, echo=FALSE, out.width='100%', out.height='220pt', fig.cap='Spark computes while R collects results', fig.align = 'center', eval = eval_code}
render_nomnoml("
#direction: right
#arrowSize: 0.4
#lineWidth: 1
...
Sometimes we may need to perform an operation not yet available through `dplyr` and `sparklyr`.  Instead of downloading the data into R so that we can perform the operation, there may be a way to use a Hive function within Spark to accomplish what we need.  The next section will cover this scenario.

### Hive Built-in Functions

Spark SQL is based on Hive's SQL conventions and functions.  It is possible to call Hive query functions within the `dplyr` interface. This means that we can use Hive functions to accomplish operations that may not be available via `dplyr`.  The Hive functions can be accesses ed within the `dplyr` verb, by calling them as if they were R functions.  Instead of failing, `dplyr` passes functions it does not recognize "as-is" to the query engine.  This gives us a lot of flexibility on the function we can use from Hive SQL.  

The Hive SQL's *percentile* function returns the exact percentile of a column in a group.  The function expects a column name, and either a single percentile value, or an array of multiple percentile values. 

```{r}
cars %>%
  summarise(
    mpg_percentile = percentile(mpg, 0.25)
    )
```
...
cars %>%
  summarise(
    mpg_percentile = percentile(mpg, array(0.25, 0.5, 0.75))
    )
```
...
We have included a comprehensive list of all the Hive functions. It is available in the Appendix, under [Hive functions](#hive-functions)
...
We can pipe the results to other `corrr` functions. For example, the `shave()` functions turns all of the duplicated results into `NA`’s.  Additionally, the results can be easily visualized using the `rplot()` function, it returns a `ggplot` object. Figure \@ref(fig:analysis-rplot) is an example the plot resulting of combining these two functions. 
...
The `dbplot` package provides helper functions for plotting with remote data.  The R code `dbplot` uses to transform the data is written so that it can be translated by `sparklyr` into SQL.  It then uses those results to create a graph using the `ggplot2` package.  The data transformation, and the plotting are both triggered by a single `dbplot` function.  
...
As we covered in this section, `dbplot` offers the convenience of pushing the calculations to Spark, and also to automatically plot the results.  If desired, `ggplot2` functions can be used directly, the plot's calculations would have to be coded by you.  For small and local data sets, either `dbplot` or `ggplot2` will perform well. 

We use statistical models to gain even deeper insights into our data.  The next section will cover how we can prepare and run models within Spark.  

## Model

An analysis project focuses on going through as many transformations and models to find the answer.  The ideal data analysis framework enables the user to quickly complete each iteration. The transition from an analysis to a deployment project occurs after the model is selected, and findings are presented to the stakeholders. [Modeling](#modeling) will dive deeper into how to prepare and run models.  The focus of this section will be how to properly, and easily, transition from wrangling to modeling.
