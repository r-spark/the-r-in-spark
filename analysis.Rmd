...
Most of these outputs are available in the core R packages of R Markdown: `knitr` and `rmarkdown`.  Other output formats are made available by companion R packages. For example, the package `xaringan` adds the ability to create great looking presentation decks.  This book was written using R Markdown thanks to an extension provided by the `bookdown` package.  The best resource to delve deeper into R Markdown is the official book [@r-markdown-the-definite-guide].

In R Markdown, one type of artifact could potentially be rendered in different formats.  For example, the same report could be rendered in HTML, or as a PDF file by changing a setting within the report itself.  Conversly, multiple types of artifacts could be rendered as the same output.  For example, a presentation deck and a report could be rendered in HTML.
...
The `sparklyr` package implements data analysis principles of R and specifically a subset of R packages called the `tidyverse`.  If you are new to R, Data Science, or the `tidyverse`, the book "R for Data Science" [@intro-r-for-data-science] provides the best introduction to these concepts. 
