...
In a enterprise setting, `copy_to()` should only be used to transfer small tables from R, such as a look up value table.
...
A very common exercise is to run correlation calculations of the variables among the data.  The Spark API contains an internal function that runs the correlations inside Spark, and returns the results as a `data.frame`.
...
```{r}
mc %>%
  mutate(var2 = colnames(mc)) %>%
  gather(key = "var1", value = "cor", - var2) %>%
