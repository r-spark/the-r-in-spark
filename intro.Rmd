...
The world bank report on digital development provides an estimate of digital and analog information stored over the last decades. [@intro-data-revolution] This report noted that digital information surpassed analog information around 2003. At that time, there were about 10 million terabytes of digital information, which is roughly about 10 million computer drives today. However, a more relevant finding from this report was that our footprint of digital information is growing at exponential rates. Figure \@ref(fig:intro-store-capacity) shows the findings of this report, notice that every other year, the worlds information has grown tenfold.
...
While Spark is well known for its in-memory performance, Spark was designed to be a general execution engine that works both in-memory and on-disk. For instance, Spark has set new records in large-scale sorting [@intro-sort-benchmark], where data was not loaded in-memory; but rather, Spark made use of improvements in network serialization, network shuffling and efficient use of the CPU's cache to dramatically improve performance. If you need to sort large amounds of data, at the time, there was no other system in the world faster than Spark.

To give you a sense of how much fasteer and efficient Spark is, you can sort 100 terabytes of data in 72min and 2100 computers using Hadoop, but only 206 computers in 23 minutes using Spark [@intro-spark-record-sorting]. In addition, Spark holds the cloud sorting benchmark [@intro-spark-record-cloudsort], which made Spark the most cost effective solution for sort large-datasets in the cloud.
...
In addition, Spark is much more simple to use than Hadoop; for instance, the word count MapReducee example takes about fifty lines of code in Hadoop, but it only takes two lines of code in Spark -- Spark is much faster, efficient and easier to use than Hadoop.
...
> "Apache Spark is a unified analytics engine for large-scale data processing    is a fast and general engine for large-scale data processing."
>
...
Unified
: Spark supports many libraries, clusters technologies and storage systems with ease.

Analytics
: Analytics is the discovery and interpretation of data to produce and communicate meaningful information.

Engine
: An engine is expected to be efficient and generic.

Large-Scale
: One can interpret this as *cluster*-scale, as in, a set of connected computers working together to accomplish specific goals.

The definition describes Spark as an _engine_, since it optimizes and executes generic code; there are no restrictions as to what type of code one can write in Spark. Spark is also quite efficient, as we mentioned, it's much faster than its predecessor by making efficient use of memory, network and CPUs to speed data processing algorithms in computing clusters.

Therefore, Spark is used in many _analytics_ projects, like ranking movies at Netflix [@intro-spark-churn-prediction], aligning protein sequences [@intro-spark-bioinformatics] or analyzing high energy physics at CERN. [@intro-spark-cern] 

As _unified_ platform, Spark is expected to support many cluster technologies and multiple data sources, which you will learn about in the Clusters and Data chapters. It is also expected to support many different libraries like Spark SQL, MLlib, GraphX and Spark Streaming; libraries which are used for analysis, modeling, graph processing and real-time data processing respectevely. Conceptually, you can think of the unified funcitonality Spark provides through Figure \ref(@intro-spark-unified).

```{r intro-spark-unified, echo=FALSE, out.width='100%', out.height='220pt', fig.cap='Spark as a unified analytics engine for large-scale data processing', fig.align = 'center', eval = eval_code}
render_nomnoml("
#direction: down
#fontSize: 16
#padding: 16
...
Describing Spark as _large scale_ implies that a good use case for Spark is tackling problems that can be solved with multiple machines. For instance, when data does not fit in a single disk driver or does not fit into memory, Spark is a good candidate to consider. However, it can also be considereed for problems that may not be large-scale, but where using multiple computers could speed up computation. For instance, CPU intensive models or scientific simulations also bennefit from running in Spark.
...
This chapter presented Spark as a modern and powerful computing platform, R as an easy-to-use computing language with solid foundations in statistical methods and, `sparklyr`, as a project bridging both technologies and communities together. In a world where the total amount of information is growing exponentailly, learning how to analyze data at scale will help you tackle the problems and opportunities humanity is facing today. However, before we start analzing data, the Getting Started chapter will equip you with the tools you will need through the rest of this book. We recommend you follow each step carefully and take the time to install the recommended tools which, we hope will become familiar tools that you use and love.
