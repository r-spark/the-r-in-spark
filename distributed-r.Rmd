...
**Warning:** If you are already a familiar R user, you might be tempted to use this approach for all Spark operations; however, this is not the recommended use of `spark_apply()`. Previous chapters provided more efficient techiniques and tools to solve well known problems, in contrast, `spark_apply()` introduces additional cognitive overhead, additional troubleshooting steps, performance tradeoofs and, in general, additional complexity that should be avoided. Not to say that `spark_apply()` should never used; but instead, `spark_apply()` is reserved to support use cases where previous tools and techniques fall short.
