...
Since `spark_extension()` is registered as a custom project extension in RStudio; you can also create an R package that extends Spark using Scala code from the `File` menu, `New Project...` and selecting `R Package using sparklyr` as shown in Figure \@ref(fig:contributing-r-rstudio-project).
